...
Do ponto de vista acadêmico, o projeto oferece ao grupo a oportunidade de aplicar, em um cenário real, conceitos estudados ao longo do curso de Tecnologia da Informação: modelagem de banco de dados relacional com restrições de integridade (ELMASRI; NAVATHE, 2011), desenvolvimento web seguindo o padrão arquitetural MTV do Django (DJANGO SOFTWARE FOUNDATION, 2024), conteinerização de aplicações com Docker (DOCKER INC., 2024), construção de API RESTful (FIELDING, 2000) e práticas de integração contínua (HUMBLE; FARLEY, 2010). A intersecção entre engenharia de software e gestão escolar permite que o grupo vivencie o ciclo completo de desenvolvimento, desde a elicitação de requisitos junto ao usuário final até o deploy e a coleta de \textit{feedback}.
...
Um dos mecanismos centrais para garantir a integridade dos dados no Leciona é o uso de \textit{constraints} (restrições) definidas diretamente no banco. O modelo Schedule, por exemplo, possui duas restrições de unicidade (\texttt{UniqueConstraint}): a primeira impede que um professor seja alocado em duas turmas diferentes no mesmo dia e horário (\texttt{unique\_teacher\_schedule\_time}); a segunda impede que uma turma receba duas aulas simultâneas (\texttt{unique\_classgroup\_schedule\_time}). Essas restrições são aplicadas pelo PostgreSQL no momento da inserção ou atualização dos dados, independentemente da camada de aplicação. Isso significa que, mesmo que uma validação na View ou no formulário falhe por algum motivo, o banco de dados impede a criação de registros inconsistentes (ELMASRI; NAVATHE, 2011).
...
Essa observação, que remonta ao trabalho de Dijkstra, reforça a importância de combinar testes automatizados com outras estratégias de verificação, como revisão de código e \textit{constraints} de banco de dados, para maximizar a cobertura de detecção de erros.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{images/diagrama-er.png}
...
% NOTA: Inserir screenshot do dashboard aqui.
% \begin{figure}[H]
% \centering
...
% NOTA: Inserir screenshot do painel administrativo aqui.
% \begin{figure}[H]
% \centering
...
% NOTA: Inserir screenshot de um PDF exportado aqui.
% \begin{figure}[H]
% \centering
% \includegraphics[width=0.85\textwidth]{images/pdf-export.png}
% \caption{Exemplo de relatório em PDF gerado pelo sistema Leciona}
% \label{fig:pdf}
% \end{figure}
...
% NOTA: Inserir screenshot do cadastro de horários aqui.
% \begin{figure}[H]
% \centering
% \includegraphics[width=0.9\textwidth]{images/schedule-management.png}
% \caption{Formulário de cadastro de horários com validação de conflitos}
% \label{fig:schedule}
% \end{figure}
